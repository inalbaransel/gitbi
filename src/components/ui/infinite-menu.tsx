  ...
  private initTexture(): void {
    ...
    Promise.all(
      this.items.map(
        item =>
          new Promise<HTMLImageElement>(resolve => {
            ...
            img.onerror = () => {
              console.warn('Failed to load image (CORS or 404):', item.image);
              // Provide a tiny transparent fallback to keep the atlas valid
              const canvas = document.createElement('canvas');
              canvas.width = 1;
              canvas.height = 1;
      ...
      images.forEach((img, i) => {
        ...
        // Cover logic: find the largest centered square in the original image
        const imgW = img.width;
        const imgH = img.height;
        const size = Math.min(imgW, imgH);
        const offsetX = (imgW - size) / 2;
        const offsetY = (imgH - size) / 2;

        ctx.drawImage(img, offsetX, offsetY, size, size, x, y, cellSize, cellSize);
      });
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [], scale = 1.0 }) => {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {activeItem && (
        <div className={`absolute bottom-32 md:bottom-20 left-1/2 -translate-x-1/2 w-full text-center pointer-events-none transition-all duration-700 cubic-bezier(0.19, 1, 0.22, 1) ${isMoving ? 'opacity-0 translate-y-10 scale-95' : 'opacity-100 translate-y-0 scale-100'}`}>
          <h2 className="text-4xl md:text-6xl font-wc-rough-trad text-[#f2f3f7] tracking-wider drop-shadow-[0_4px_12px_rgba(0,0,0,0.8)]">
            {activeItem.title}
          </h2>
